<template>
  <div>
    <v-app-bar app elevate-on-scroll>
      <v-container>
        <v-toolbar flat>
          <v-app-bar-nav-icon @click.stop="drawer = !drawer" class="d-sm-none d-flex"></v-app-bar-nav-icon>
          <v-spacer class="d-sm-none d-flex"></v-spacer>
          <div>
            <img src="../assets/Logo.png" @click="$router.push('/').catch(err => {})" id="logo" />
          </div>
          <v-toolbar-title
            @click="$router.push('/').catch(err => {})"
            class="gamjaFont px-4"
            id="quantify"
          >퀀티파이</v-toolbar-title>
          <v-toolbar-items class="d-sm-flex d-none">
            <v-btn @click="$router.push('/about').catch(err => {})" class="nanumFont" text>소개</v-btn>
            <v-btn @click="$router.push('/list').catch(err => {})" class="nanumFont" text>전체종목</v-btn>
          </v-toolbar-items>
          <v-spacer></v-spacer>
          <v-btn v-if="state" @click.stop="dialog = true" class="d-sm-none d-flex" icon>
          ...
          <v-toolbar-items class="d-sm-flex d-none align-center">
            <v-btn v-if="state" @click.stop="dialog = true" class="nanumFont" text>로그인</v-btn>
            <v-row v-else>
              <div class="my-auto">
                <p
                  @click="$router.push('/user').catch(err => {})"
              ...
              <v-btn @click.stop="logout()" class="nanumFont" text>로그아웃</v-btn>
            </v-row>
    ...
    <v-navigation-drawer v-model="drawer" class="d-sm-none d-flex" fixed temporary>
      <v-list-item v-if="state">
        <v-list-item-content>
          <v-btn @click.stop="dialog = true" class="nanumFont" color="#3e9278" dark>로그인</v-btn>
        </v-list-item-content>
      </v-list-item>
      <v-list-item v-else>
        <v-list-item-avatar>
        ...
        <v-list-item-content>
          <v-list-item-title class="title nanumFont">{{username}}</v-list-item-title>
        </v-list-item-content>
      </v-list-item>
      ...
      <template v-if="state==false" v-slot:append>
        <div class="pa-2">
          <v-btn @click.stop="logout()" class="nanumFont" color="#3e9278" block dark>로그아웃</v-btn>
        </div>
      </template>
    </v-navigation-drawer>
  </div>
...
export default {
  ...
  methods: {
    stop() {
      window.open(
        "http://localhost:8080/notWorking",
        "window팝업",
