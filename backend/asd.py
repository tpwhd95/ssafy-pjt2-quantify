...
class Backtest():
    ...
    def buy_stock(self,date):
        ...
        for i in range(self.divide):
            ...
            self.log.append({"date": date.strftime('%Y-%m-%d'),"type":"buy","datas":self.stock_list.copy()})

    def sell_stock(self,date):
        print("sell_start")
        self.log.append({"date": date.strftime('%Y-%m-%d'),"type":"sell","datas":self.stock_list.copy()})
